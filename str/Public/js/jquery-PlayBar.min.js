...
(function($) {
    jQuery.playBar = {
        addBar: function(DOM, allTime) {
            ...
            $(".BarControl").append('<div class="TheBar"><div class="TimeBall"></div><div class="TheColorBar"></div></div><div class="BarFinishTime">01:00:00</div>');
            width = $('.TheBar').width();
            times = allTime / 1000;
            rwidth = width - 8;
    ...
    function CleanAll() {
		if(startSecond>0){
			thewidth=parseInt((startSecond*$('.TheBar').width()-8)/3600);
			CurrTime = 0;
			addHour = 0;
			addMinute = parseInt(startSecond/60);
			addSecond = parseInt(startSecond%60);
		}else{
			thewidth = 0;
			CurrTime = 0;
			addHour = 0;
    ...
    $(document).mouseup(function() {
        if (BarMove) {
			...
			seek(CurrTime);
			checkStatus2=setInterval(checkStatusDrag,200)
 /*           if (isAction) {
                OpenBar()
            }*/
        }
