...
class PublicAction extends Action
{
	...
	*/
    public function getAlarmMessage(){
        ...
        $video=$model->query("select * from t_alarm_message where alarmHappenTime>='$time[time]'");
        //保存到自己的数据库中
        ...
        $radio=$model2->query("select * from t_alarm_message where alarmHappenTime>='$time[time]'");
        //保存到自己的数据库中
        foreach($radio as $v){
    ...
    */
    public function TVSubmit($data){
        ...
        $res=$client->TVSubmit($params);
        $AlarmSend=M('alarm_send');
		unset($client);
        if($res->TVSubmitResult->Success=="true"){
            $alarmId=$data['alarmId'];
            $AlarmSend->where("alarmID='$alarmId'")->setField("workflowID",$res->TVSubmitResult->workflowID);
            return 1;
        }else{
            return 0;
        }
        //$res->TVSubmitResult->Success;
        //$res->TVSubmitResult->workflowID;
    }
