    ...
    */
    ...
    public function ajaxGetMessage(){
        ...
        if($_POST['condition']==1){
            ...
            if($_POST['type']!=""){
                ($_POST['type']!=2)?$condition.=" and type='$_POST[type]'":'';
            }
        ...
        foreach($list as $v){
            ...
            $user.="<tr><td>".$v[date]."</td><td>".$v[time]."</td><td>".$type."</td>
                <td style='width:800px;'>".$v[content]."</td><td>".$v[groupName]."</td><td>".$status."</td><td>".$v[code]."</td>
                   <td><button type='button' class='btn btn-warning' style='width:100px;margin-right:20px;' id='".$v[sendid]."'>查看详情</button></td></tr>";
        }
        $data[0]=$show;$data[1]=$user;
        $this->ajaxReturn($data,'',1);
    }
